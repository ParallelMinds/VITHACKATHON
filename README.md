...
| `llm_assistant.py` | Local rule-based explain-image engine + template fallback |
...
### 2. Configure Explain Image Mode (No external API required)
The system uses local rule-based explanation and image insight generation.
No external generative model key is required.
...
| Explain mode issue | Disable the explain toggle to run detection-only mode. Template fallback always works. |
