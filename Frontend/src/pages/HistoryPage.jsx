...
/* ── SHAP Explanation Panel (rich display for shap_explainer steps) ────── */

const ShapExplanationPanel = ({ output }) => {
  if (!output || typeof output !== 'object') return null;

  const verdictColor =
    output.icon === '🚨' ? 'border-red-200 bg-red-50 text-red-800'
    : output.icon === '⚠️' ? 'border-amber-200 bg-amber-50 text-amber-800'
    : output.icon === '✅' ? 'border-emerald-200 bg-emerald-50 text-emerald-800'
    : 'border-slate-200 bg-slate-50 text-slate-700';

  const strengthColor =
    output.strength === 'High' ? 'text-red-700 bg-red-50 border-red-200'
    : output.strength === 'Medium' ? 'text-amber-700 bg-amber-50 border-amber-200'
    : 'text-slate-600 bg-slate-50 border-slate-200';

  const reliabilityColor =
    output.reliability === 'High' ? 'text-emerald-700 bg-emerald-50 border-emerald-200'
    : output.reliability === 'Moderate' ? 'text-amber-700 bg-amber-50 border-amber-200'
    : 'text-slate-600 bg-slate-50 border-slate-200';
  ...
  return (
    <div className="space-y-3">
      {/* Verdict banner */}
      {output.verdict && (
        <div className={`rounded-lg border px-3 py-2.5 ${verdictColor}`}>
          <div className="flex items-start gap-2">
            <span className="text-base shrink-0">{output.icon}</span>
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-wide mb-0.5">SHAP Verdict</p>
              <p className="text-sm leading-snug whitespace-pre-line">{output.verdict}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Detected object row */}
      {output.label && (
        <div className="flex items-center gap-3 rounded-lg border border-slate-100 bg-white/80 px-3 py-2">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Detected Object</p>
            <p className="text-sm font-semibold text-slate-900 mt-0.5">
              {output.label}
              <span className="ml-2 text-xs font-medium text-slate-500">
                {output.confidence != null ? `${(output.confidence * 100).toFixed(1)}% conf.` : ''}
              </span>
            </p>
          </div>
          {output.is_suspicious != null && (
            <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase border ${
              output.is_suspicious ? 'bg-red-50 text-red-700 border-red-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
            }`}>
              {output.is_suspicious ? 'Suspicious' : 'Safe'}
            </span>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className={`rounded-lg border p-2 text-center ${strengthColor}`}>
          <p className="text-[10px] font-bold uppercase tracking-wide opacity-70">Strength</p>
          <p className="text-sm font-bold mt-0.5">{output.strength ?? '—'}</p>
        </div>
        <div className={`rounded-lg border p-2 text-center ${reliabilityColor}`}>
          <p className="text-[10px] font-bold uppercase tracking-wide opacity-70">Reliability</p>
          <p className="text-sm font-bold mt-0.5">{output.reliability ?? '—'}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-center">
          <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Coverage</p>
          <p className="text-sm font-bold text-slate-900 mt-0.5">
            {output.coverage != null ? `${output.coverage.toFixed(1)}%` : '—'}
          </p>
        </div>
      </div>

      {/* Attribution details */}
      {(output.mean_positive_attribution != null || output.mean_negative_attribution != null) && (
        <div className="rounded-lg bg-slate-50/80 border border-slate-100 px-3 py-2 space-y-1">
          <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-1">Attribution Details</p>
          <div className="flex items-start gap-2 text-xs">
            <span className="font-semibold text-slate-500 shrink-0 min-w-[140px]">Positive attribution:</span>
            <span className="text-emerald-700 font-medium">{output.mean_positive_attribution?.toFixed(6) ?? '—'}</span>
          </div>
          <div className="flex items-start gap-2 text-xs">
            <span className="font-semibold text-slate-500 shrink-0 min-w-[140px]">Negative attribution:</span>
            <span className="text-red-700 font-medium">{output.mean_negative_attribution?.toFixed(6) ?? '—'}</span>
          </div>
          {output.peak_attribution_region && (
            <div className="flex items-start gap-2 text-xs">
              <span className="font-semibold text-slate-500 shrink-0 min-w-[140px]">Peak region:</span>
              <span className="text-slate-800">({output.peak_attribution_region[0]}, {output.peak_attribution_region[1]})</span>
            </div>
          )}
          {output.shap_intensity_score != null && (
            <div className="flex items-start gap-2 text-xs">
              <span className="font-semibold text-slate-500 shrink-0 min-w-[140px]">Intensity score:</span>
              <span className="text-slate-900 font-bold">{(output.shap_intensity_score * 100).toFixed(1)}%</span>
            </div>
          )}
        </div>
      )}
...
const StepCard = ({ step, index }) => {
  ...
  const isShapStep = step.service === 'shap_explainer' || step.service === 'shap_service';
  const hasShapExplanation = isShapStep && step.output && step.output.verdict;
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(step.service === 'shap_explainer' || step.service === 'shap_service') && step.status === 'success' && step.output?.strength && !open && (
        ...
        {open && (
          ...
          >
            <div className="px-4 py-3 space-y-2">
              ...
              {/* Use rich SHAP panel when available, otherwise generic display */}
              {hasShapExplanation ? (
                <div className="mt-2">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-2">SHAP Explanation</p>
                  <ShapExplanationPanel output={step.output} />
                </div>
              ) : (
