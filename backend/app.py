...
Flask Web Application with local rule-based explain-image analysis
"""
...
def status():
    from inference import MODEL_A, MODEL_B
    return jsonify({
        "status": "online",
        "models_loaded": list(models.keys()),
        "version": "3.0.0-aegis",
        "detection_model": f"{MODEL_A['label']} + {MODEL_B['label']} (fallback)",
...
def analyze():
    ...
    use_explain_mode = request.form.get("use_llm", "false").lower() == "true"
    ...
    try:
        ...
        # Local rule-based vision analysis
        ...
        # Local rule-based text explanation
        if use_explain_mode:
            explain_result = generate_explanation_sync(result)
            result["explanation"] = explain_result["explanation"]
            result["explanation_source"] = explain_result["source"]
            result["llm_latency_ms"] = explain_result["latency_ms"]
        else:
        ...
        response = {
            ...
            "declaration_nlp": result.get("declaration_nlp", {}),
