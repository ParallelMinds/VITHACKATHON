...
def _extract_shap_explanation(
    ...
    """
    Run the local SHAP explainer and return the full explanation dict.

    Returns a JSON-serialisable dict with keys:
      label, confidence, is_suspicious, strength_label, reliability,
      high_attribution_coverage_pct, verdict, icon,
      mean_positive_attribution, mean_negative_attribution,
      peak_attribution_region, shap_intensity_score.
    ...
    try:
        ...
        # Build a serialisable explanation dict (drop numpy pixel_importance_map)
        intensity = min(conclusion["high_attribution_coverage_pct"] / 100.0, 1.0)
        return {
            "label": conclusion["label"],
            "confidence": round(conclusion["confidence"], 4),
            "is_suspicious": conclusion["is_suspicious"],
            "strength": conclusion["strength_label"],
            "reliability": conclusion["reliability"],
            "coverage": round(conclusion["high_attribution_coverage_pct"], 2),
            "verdict": conclusion["verdict"],
            "icon": conclusion["icon"],
            "mean_positive_attribution": round(conclusion["mean_positive_attribution"], 6),
            "mean_negative_attribution": round(conclusion["mean_negative_attribution"], 6),
            "peak_attribution_region": list(conclusion["peak_attribution_region"]),
            "shap_intensity_score": round(intensity, 4),
        }
    except Exception as exc:
...
async def analyze(
    ...
    try:
        ...
        try:
            ...
            if shap_data is not None:
                ...
                shap_explanation = await asyncio.to_thread(
                    _extract_shap_explanation, detector, image
                )
                if shap_explanation is not None:
                    shap_intensity_score = shap_explanation["shap_intensity_score"]
                    outputs["shapIntensityScore"] = shap_intensity_score
                    outputs["shap"] = shap_explanation
                    audit.log_step(
                        service="shap_explainer",
                        status="success",
                        input_data={"source": "local_fallback"},
                        output_data=shap_explanation,
                        latency=time.time() - t0,
                    )
                else:
                    audit.log_step(
        ...
        # Prefer VLM-extracted item names when available; fall back to regex.
        if vlm_result and vlm_result.get("extracted_items"):
            vlm_item_names = [
                item.get("item_name", "")
                for item in vlm_result["extracted_items"]
                if item.get("item_name")
            ]
            outputs["manifestItems"] = vlm_item_names if vlm_item_names else manifest_items
            outputs["vlmResult"] = vlm_result
        elif manifest_items:
